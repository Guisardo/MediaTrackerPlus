...
describe('RecommendationService - metadataProviders dispatch', () => {
  ...
  it('calls metadataProviders.similar() with the resolved mediaItem for tv', async () => {
  ...
  it('calls metadataProviders.similar() for video_game', async () => {
    const items = makeSimilarItems(5, 'video_game');
    const deps = makeDeps();
  ...
  it('uses empty array when metadataProviders.similar() returns null (unsupported type)', async () => {
    const deps = makeDeps();
...
describe('RecommendationService - error handling', () => {
  ...
  it('swallows error from metadataProviders.similar() and does not rethrow', async () => {
    ...
    const apiError = new Error('TMDB 500 Internal Server Error');
    (deps.findMediaItemById as jest.Mock).mockResolvedValue(mediaItem);
    (deps.metadataProviders.similar as jest.Mock).mockRejectedValue(apiError);
    const service = new RecommendationService(deps);
    await expect(service.processRating(1, 42, 8)).resolves.toBeUndefined();
  });

  it('logs error from metadataProviders.similar() at ERROR level with err object', async () => {
    const deps = makeDeps();
...
describe('RecommendationService - edge cases', () => {
  ...
  it('returns early and logs WARN when mediaItem is not found', async () => {
    ...
    expect(deps.metadataProviders.similar).not.toHaveBeenCalled();
  ...
  it('passes empty array to WatchlistWriter when similar() returns null', async () => {
    const deps = makeDeps();
    const mediaItem = makeMediaItem({ mediaType: 'audiobook' });
    (deps.findMediaItemById as jest.Mock).mockResolvedValue(mediaItem);
    (deps.metadataProviders.similar as jest.Mock).mockReturnValue(null);
  ...
  it('passes empty array to WatchlistWriter when similar() returns empty array', async () => {
    const deps = makeDeps();
    const mediaItem = makeMediaItem({ mediaType: 'movie', tmdbId: 200 });
    (deps.findMediaItemById as jest.Mock).mockResolvedValue(mediaItem);
    (deps.metadataProviders.similar as jest.Mock).mockResolvedValue([]);
