...
describe('mediaItemRepository.items — group-scoped majority-watched exclusion (US-010)', () => {
  ...
  beforeAll(async () => {
    ...
    await Database.knex('userGroup').insert({
      id: groupId,
      name: 'Majority Watched Test Group',
      createdBy: user1.id,
      createdAt: Date.now(),
    });
